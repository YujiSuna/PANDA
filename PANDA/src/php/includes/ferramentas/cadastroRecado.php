...
$allRecados = $cRecado->getAllRecados();
...
"<script>
    ...
    const allRecados = " . json_encode($allRecados) . ";
    console.log(allRecados);
    const allTurmas = " . json_encode($allTurmas) . ";
    const allAlunos = " . json_encode($allAlunos) . ";
    const recadoLink = " . json_encode($_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']) . "
</script>";
...
if (!(isset($_GET['bGFzdEFkZGVkUmVjYWRvSWQNCg']) && $_GET['bGFzdEFkZGVkUmVjYWRvSWQNCg'] > 0)) {
    ...
    $idRecadoCriado = intval($_GET['bGFzdEFkZGVkUmVjYWRvSWQNCg']);
    $recadoCriado = $cRecado->getRecado($idRecadoCriado);
    $formNum = 2;
}
?>
        ...
        <div id="card-cadastro" class="card">
            <nav>
                <div class="nav nav-tabs justify-content-around" id="nav-tab" role="tablist">
                    <button class="col-6 nav-link active" id="nav-1-tab" data-bs-toggle="tab" data-bs-target="#nav-1" type="button" role="tab" aria-controls="nav-1" aria-selected="true"><strong>cadastro-recado</strong></button>
            ...
            <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane show active" id="nav-1" role="tabpanel" aria-labelledby="nav-1-tab">
                    <div id="recadoCardContent" class='cardContent mx-3 py-3'>
                        <?php include_once '/xampp/htdocs/PANDA/src/php/includes/ferramentas/cadastros/cadastro-recadoForm1.php'; ?>
                    </div>
                </div>

                <div class="tab-pane" id="nav-2" role="tabpanel" aria-labelledby="nav-2-tab">
                    <div id="recadoCardContent" class='cardContent mx-3 py-3'>
                        <?php include_once '/xampp/htdocs/PANDA/src/php/includes/ferramentas/cadastros/cadastro-recadoForm2.php'; ?>
                    </div>
                </div>
            </div>
        </div>
    ...
    navLinks.forEach(navLink => {
        navLink.addEventListener("click", (evennt) => {
            if (navLink.classList.value.includes("active")) {
                ...
                setAJAXrecado(<?php echo $formNum ?>);
            }
        });
    });
</script>
